...
void Program::SetUniform(const std::string& name, const sglm::vec2& value) const {
    auto loc = glGetUniformLocation(m_program, name.c_str());
    glUniform2fv(loc, 1, sglm::value_ptr(value));
}
...
void Program::SetUniform(const std::string& name, const sglm::vec4& value) const {
    auto loc = glGetUniformLocation(m_program, name.c_str());
    glUniform4fv(loc, 1, sglm::value_ptr(value));
}
